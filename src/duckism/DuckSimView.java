    ...
    extends JPanel
{
    ...
    public boolean xyIsWithinRect(
        ...
        int rectH)
    {
        return (rectX <= x && x <= rectX + rectW && rectY <= y
            && y <= rectY + rectH);
    }
    ...
    public void paintDuck(Graphics g, int pos)
    {
        ...
        if (model.getCurrentDuck() == pos
            && (flyTimer.isRunning() || quackTimer.isRunning()))
        {
        ...
        // If the duck is selected, paint the body of the square the same color
        // as the duck, otherwise paint it white.
        ...
        {
            // When a duck joins the welcoming committee, a little "w" should
            // appear in the bottom left corner of the duck's square.
            g.setColor(Color.MAGENTA);
            totalWidth = fm.stringWidth("w!");
            g.drawString("w", x + 25 - totalWidth / 2, y + 85);

            // If a duck is captured, it displays "beware!"
            if (!model.getDuck(pos).isFree()
                && model.getDuck(pos).getState() == State.WELCOMING)
            {
            ...
            else if (model.getDuck(pos).getState() == State.WELCOMING)
            {
        ...
        // If the duck is captured, put a red X in the lower right corner of
        // the square.
        if (!model.getDuck(pos).isFree())
        {
